...
class OpenAIRecoveryAgent:
    ...
    def propose_pointers(self, view: dict) -> list[str]:
        ...
        sources = view.get("evidence_sources_available", [])
        if not isinstance(sources, (list, tuple)) or not all(isinstance(source, str) for source in sources):
            self._fail("invalid_input")
        allowed = {f"{source}:manifest" for source in sources}
        order_id = view.get("order_id")
        if isinstance(order_id, str) and order_id:
            allowed.update(f"{source}:order:{order_id}" for source in sources)
        # A resolvable pointer can still name the wrong order or an unlisted
        # source. Enforce the supplied view instead of trusting the prompt.
        if any(pointer not in allowed for pointer in result["pointers"]):
            self._fail("invalid_pointers")
