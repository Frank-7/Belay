"""Shared, read-only payment evidence checks; no wallet or execution capability."""
...
SCHEMA = "belay.payment.investigation.v1"
...
class PaymentIntent:
    ...
    def __post_init__(self):
        if type(self.revision) is not int or self.revision < 0:
            raise ValueError("Invalid payment revision")
        for field in ("run_id", "mission_id", "operation_id", "beneficiary_id"):
        ...
        if not isinstance(self.grant_digest, str) or not re.fullmatch(r"sha256:[a-f0-9]{64}", self.grant_digest):
            raise ValueError("Invalid payment authority digest")
        ...
        if not isinstance(intent, dict) or canonical(intent) != self.intent_json:
            raise ValueError("Payment intent must be a canonical object")
        expected = {
            "operation_id": self.operation_id, "grant_digest": self.grant_digest,
            "payee_id": self.beneficiary_id, "source_asset": "USDC",
            "destination_asset": "USD", "source_usdc_units": self.source_usdc_units,
            "destination_usd_cents": self.net_usd_cents,
        }
        if any(canonical(intent.get(key)) != canonical(value) for key, value in expected.items()):
            raise ValueError("Payment intent fields disagree")
    ...
    def view(self):
        ...
        result.update(source_asset="USDC", source_decimals=6,
                      ...
                      intent_digest=digest(json.loads(self.intent_json)))
        return result


def blocked_finding(run_id, revision, reason, *, schema=SCHEMA, provider="mission_simulator"):
    return {
        ...
        "provider": provider, "mode": "deterministic",
        "scope": "Fictional USD payout evidence; no money moved",
    }


def investigate(intent, provider_record: dict | None, *, unavailable=False, schema=SCHEMA,
                provider="mission_simulator", domain="payment"):
    """Shared deterministic evidence checks for typed purchase and payment intents."""
    ...
    try:
        ...
        source = provider.removesuffix("_simulator") + "_provider"
        observation = Observation(
            Pointer(source, "operation:" + intent.operation_id),
            {"source": source, "operation": record},
            "fictional-provider:operation:" + intent.operation_id,
        )
        result["observations"] = [{"source": source, **observation.as_dict()}]
        checks = []
        ...
        check("Exact saved authority and payment instruction", record.get("intent_json") == intent.intent_json)
        check("Bound beneficiary", record.get("beneficiary_id") == intent.beneficiary_id)
        ...
        if not identity_ok or (record.get("payout_state") == "paid" and not all(item["passed"] for item in checks)):
            result.update(verdict="conflict", summary="Provider evidence disagrees with the saved " + domain + ". Reconciliation is blocked.")
        elif all(item["passed"] for item in checks):
            result.update(verdict="paid", can_reconcile=True, citations=[observation.digest],
                          summary="The original USD payout is confirmed in the fictional provider. Reconcile it without sending again. Delivery, service and other external outcomes remain separate.")
        else:
