...
Belay's AI Apps promise is authorized agent actions that can be explained and
safely recovered. Payment Mission demonstrates the commerce journey, including
lost-payout investigation through Recovery Desk's shared read-only validator.
The separate Recovery Desk app retains its optional model and human-signed
testnet path. Distinguish these local tools from recordings and future live
...
| [Recovery Desk presentation](FINAL_PRESENTATION.md) | Alternate demonstration of the same AI Apps promise | Optional model, testnet evidence, judge questions and buyer questions |
...
`belay.mission.v0.1` path uses its own typed adapter over the same validator. Recovery
