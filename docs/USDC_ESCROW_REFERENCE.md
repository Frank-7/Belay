...
**Historical v0.2 design.** Superseded by
[v0.3 USDC funding with USD supplier payout](USDC_SETTLEMENT_ARCHITECTURE.md).
The prior requirement that sellers receive USDC/use their own off-ramp no
longer applies. Retain these contract-escrow ideas only as comparison material;
the current main flow supports supplier prepayment plus separately funded
customer protection. Read [MVP_MASTER_PLAN.md](MVP_MASTER_PLAN.md) first.
